...
export default class extends Controller {
  ...
  destroyTerminal({ killRemote = true, closeRemoteBrowser = false } = {}) {
    ...
    if (killRemote && previousSessionId) {
      this.resetOpencodeTracking(previousSessionId)
      this.sessionStateById.delete(previousSessionId)
      this.runningSessionIds.delete(previousSessionId)
      const timeoutId = this.sessionTimeoutIds.get(previousSessionId)
      if (timeoutId) {
        clearTimeout(timeoutId)
        this.sessionTimeoutIds.delete(previousSessionId)
      }
    }
    this.updateBackgroundIndicator()
